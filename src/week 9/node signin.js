	...
	if (path.extname(pathname) == ".html") {
		...
		if (request.method === "POST") {
			...
			request.on("end", function() {
				...
				if (checkDuplicate(uj["username"], uj["number"], uj["phone"], uj["email"], response)) {
...
function checkDuplicate(usr, num, ph, em, res) {
	if (usr == num) {res.write("<p class='err-input'>用户名与学号不可重复</p>")}
	if (usr == ph) {res.write("<p class='err-input'>用户名与电话不可重复</p>")}
	if (usr == em) {res.write("<p class='err-input'>用户名与邮箱不可重复</p>")}
	if (num == ph) {res.write("<p class='err-input'>学号与电话不可重复</p>")}
	if (num == em) {res.write("<p class='err-input'>学号与邮箱不可重复</p>")}
	if (ph == em) {res.write("<p class='err-input'>电话与邮箱不可重复</p>")}
}